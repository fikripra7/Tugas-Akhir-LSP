<?php
  ...
  // status 1
  if (strlen($nm_file) < 1) {
    header("location: ../index.php?edit_ba=$idu&status=1");
    exit();
  }
  ...
  if (!in_array($jn_file, $kumpulan_file)) {
    header("location: ../index.php?edit_ba=$idu&status=2");
    exit();
  }

  // status 3
  $ukuran_maks = 500000; // 100kb
  if ($uk_file > $ukuran_maks) {
    header("location: ../index.php?edit_ba=$idu&status=3");
    exit();
  }
  ...
  // query sql untuk melakukan rubah data
	...
	if($query){
		echo "<script type='text/javascript'>alert('Data berhasil dirubah!')</script><meta http-equiv='refresh' content='1; url=../index.php?lartikel'>";
	}
