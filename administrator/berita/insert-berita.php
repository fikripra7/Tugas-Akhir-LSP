<?php
  ...
  // status 1
  if (strlen($nm_file) < 1) {
    header("location: ../index.php?tartikel&status=1");
    exit();
  }
  ...
  if (!in_array($jn_file, $kumpulan_file)) {
    header("location: ../index.php?tartikel&status=2");
    exit();
  }
  ...
  if ($uk_file > $ukuran_maks) {
    header("location: ../index.php?tartikel&status=3");
    exit();
  }
	...
	$sql = "INSERT INTO tbl_berita(judul,desk_singkat,detail,waktu,foto) VALUES('$jd','$sk','$dt','$tg','$nm_file')";
